...
def auto_catchup_history(year, learner):
    ...
    try:
        ...
        for _, event in finished_races.iterrows():
            ...
            if 'Testing' in race_name: continue
...
def _run_post_race_analysis(year, race_name, learner):
    ...
    try:
        ...
        actual_pos = dict(zip(session_q.results['Abbreviation'], session_q.results['Position']))
        
        # Backtest Strategies (Compare FP3 vs Reality)
        fp3_ranks = extract_session_order_robust(year, race_name, 'FP3')
        if not fp3_ranks: return
        ...
        learner.update_after_race(race_name, {}, {'qualifying': {'method': 'blend_70_30', 'mae': 2.0}})
...
def get_available_data(year, race_name, weekend_type):
    ...
    if found: logger.info(f"   ✅ Found: {', '.join(found)}")
...
def run_weekend_predictions(year, race_name):
    ...
    race_predictor = RacePredictor(driver_chars=factory.drivers, driver_chars_path=factory.driver_file, performance_tracker=tracker)
    ...
    if data['quali']:
        method, conf = 'blend', 'Post-Quali Analysis' # We predict to compare with reality
    ...
    q_result = quali_predictor.predict(
        year=year, race_name=race_name, method=method, 
        blend_weight=blend_weight, verbose=False
    )
    
    q_df = pd.DataFrame(q_result['grid'])
    print(tabulate(q_df[['position', 'driver', 'team', 'confidence']].head(10), 
                   headers="keys", tablefmt="simple", floatfmt=".1f"))
    ...
    if fp2_pace: logger.info("   ✅ Using Real FP2 Pace")
    else: logger.info("   ℹ️  Using Estimated Pace")
    ...
    print(tabulate(r_df[['position', 'driver', 'team', 'confidence', 'podium_probability']].head(10), 
                   headers="keys", tablefmt="simple", floatfmt=".1f"))
