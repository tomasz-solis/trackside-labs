...
def check_and_update_data(force_update=False):
    ...
    if missing_data or force_update:
        ...
        try:
            from scripts.extract_overtaking_likelihood import calculate_overtaking_likelihood, add_overtaking_to_tracks
            ...
            logger.error("   ! Could not import extraction scripts. Run from root directory.")
...
def run_simulation_loop(year=2026):
    ...
    calendar = ["Bahrain Grand Prix", "Saudi Arabian Grand Prix", "Australian Grand Prix", "Miami Grand Prix"]
    simulation_log = []
    ...
    for round_num, race_name in enumerate(calendar, 1):
        ...
        mock_grid = [{'driver': d, 'team': t, 'position': i+1} 
        ...
        logger.info(f"   🔮 Predicted Winner: {predicted_winner} (Confidence: {prediction['finish_order'][0]['confidence']:.1f}%)")
