...
class PriorsFactory:
    ...
    def _get_car_performance(self, team_name):
        ...
        matched_key = next((k for k in self.cars.keys() if k.upper().replace(' ', '') in norm_name or norm_name in k.upper().replace(' ', '')), None)
    ...
    def _derive_tiers_from_drivers(self):
        ...
        for team, paces in team_pace_scores.items():
            ...
            # Determine Tier
            if base_rating > 14: tier = 'top'
            elif base_rating > 9: tier = 'midfield'
            else: tier = 'backmarker'
        ...
        print(f"   📊 Derived Baselines: {', '.join([f'{t}: {d['base_rating']:.1f}' for t,d in sorted(derived_cars.items(), key=lambda x: x[1]['base_rating'], reverse=True)[:5]])}...")
