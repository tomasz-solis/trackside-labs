...
def get_fp_team_performance(year, race_name, session_type="FP3") -> Optional[Dict[str, float]]:
    """
    ...
    except (AttributeError, KeyError, ValueError, TypeError, ConnectionError) as e:
        logger.error(
            f"Failed to extract team performance from {session_type} for {race_name} ({year}): {e}. Will fall back to model-only predictions."
        )
        print(f"  Error loading {session_type}: {e}")
        return None


def blend_predictions(model_ranks, fp_performance, weight_fp=0.7) -> Dict[str, int]:
    """
    ...
    model_scores = {team: 1.0 - (rank - 1) / (num_teams - 1) for team, rank in model_ranks.items()}
...
def get_best_available_session(
    year, race_name, weekend_type="normal"
...
def predict_with_blending(year, race_name, model_predictions, weekend_type="normal", weight_fp=0.7):
    """
    ...
    session_used, fp_performance = get_best_available_session(year, race_name, weekend_type)
    ...
    # Blend predictions
    blended_ranks = blend_predictions(model_predictions, fp_performance, weight_fp=weight_fp)
...
def format_comparison(model_ranks, blended_ranks, actual_ranks=None) -> None:
    """
    ...
    if actual_ranks:
        print(
            f"{'Team':<25} {'Model':<8} {'Blended':<8} {'Actual':<8} {'Δ Model':<8} {'Δ Blend':<8}"
        )
    else:
    ...
    for team in sorted_teams:
        ...
        if actual_ranks:
            ...
            print(
                f"{team:<25} {model_rank:<8} {blend_rank:<8} {actual_rank:<8} "
...
if __name__ == "__main__":
    ...
    result = predict_with_blending(
        2025, "Bahrain Grand Prix", model_preds, weekend_type="normal", weight_fp=0.7
    )
    ...
    if result["blend_type"] == "fp_blended":
        format_comparison(result["model_predictions"], result["blended_ranks"])
