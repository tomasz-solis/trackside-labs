...
class DriverRanker:
    ...
    def __init__(self, characteristics_path: str) -> None:
    ...
    def _rank_team_drivers(
        ...
        if d1_pace is None or d2_pace is None:
            warnings.append(f"Missing {pace_metric} data for {team_name} - using equal split")
            return self._equal_split(team_name, baseline_pos, d1_abbr, d2_abbr)
    ...
    def _equal_split(
        self, team_name: str, baseline_pos: float, d1_abbr: str, d2_abbr: str
    ) -> List[DriverPrediction]:
