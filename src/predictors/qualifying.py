...
class QualifyingPredictor:
    ...
    def __init__(self, driver_ranker, data_dir="data/processed", performance_tracker=None):
        """
    ...
    def predict(
        ...
        if verbose:
            logger.debug(f"Predicting {race_name} ({weekend_type} weekend)")
            logger.debug(f"Method: {method}, Session: {session}")

        # Get team rankings based on method
        if method == "session_order":
            team_ranks, method_id = self._predict_session_order(year, race_name, session, verbose)
        elif method == "blend":
            team_ranks, method_id = self._predict_blended(
                year, race_name, session, weekend_type, blend_weight, track_data, car_data, verbose
            )
        else:  # model
        ...
        driver_preds = self.driver_ranker.predict_positions(
            team_predictions=team_ranks, team_lineups=lineups, session_type="qualifying"
        )
    ...
    def _predict_session_order(
        self, year, race_name, session, verbose
    ...
    def _predict_blended(
        self, year, race_name, session, weekend_type, blend_weight, track_data, car_data, verbose
    ):
        """Blend FP data with model predictions."""
        from src.predictors.team import rank_teams_for_track
        from src.predictors.blended import get_fp_team_performance, blend_predictions
        ...
        team_rankings = rank_teams_for_track(
            car_data, track, model_session, weekend_type_normalized
        )
        ...
        if not fp_performance:
            if verbose:
                logger.debug("No FP data, falling back to model-only")
    ...
    def _predict_model_only(self, race_name, session, weekend_type, track_data, car_data, verbose):
    ...
    def _get_expected_mae(self, method_id: str) -> float:
        ...
        mae = self.tracker.get_expected_mae(
            prediction_type="qualifying", method=method_id, window="last_10"
        )
    ...
    def _load_track_data(self, year) -> Dict:
    ...
    def _load_car_data(self, year) -> Dict:
    ...
    def _get_weekend_type(self, year, race_name, verbose: bool = False):
        """
        ...
        Args:
            ...
            verbose: Whether to print warnings on fallback
        """
        import logging
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not determine weekend type for {race_name}: {e}")
                if verbose:
                    logger.debug(f"Could not determine weekend type: {e}")

            # Conservative fallback
            logger.info(f"Using conventional fallback for {race_name}")
            return "conventional"

    def _determine_best_session(self, weekend_type) -> str:
