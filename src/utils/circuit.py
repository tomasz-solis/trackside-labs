...
def get_circuits(season: int) -> pd.DataFrame:
    ...
    for name in racetracks.circuitName:
        ...
        except (IndexError, KeyError, ValueError, ConnectionError) as e:
...
def get_all_circuits(start_year: int = 2020, end_year: int = 2025) -> pd.DataFrame:
    """
...
def get_circuit_corner_profile(
    session, low_thresh: int = 100, med_thresh: int = 170
) -> Dict[str, int]:
    """
    ...
    except (AttributeError, KeyError, ValueError, TypeError, IndexError, RuntimeError) as e:
        event = session.event.get("EventName", "Unknown")
        name = getattr(session, "name", "Unknown")
...
def build_profiles_for_season(
    ...
    *,
    ...
    try:
        from ..archive.helpers.general_utils import (
            _official_schedule,
            _session_list,
            _session_date_col,
            load_session,
    ...
    for _, ev in tqdm(
        past.iterrows(), total=len(past), desc=f"{year} events", leave=True, colour="blue"
    ):
        ...
        for sess in tqdm(sessions, desc=f"{year} {ev_name}", leave=True, colour="black"):
            ...
            try:
                ...
                if session is not None:  # FastF1
                    ...
                    lap_len = laps.groupby("driver_number")["lap_distance"].max().max()
                ...
                try:
                    alt = circuit_metadata.loc[
                        circuit_metadata["location"] == location, "altitude"
                    ].iloc[0]
                except IndexError:
                    alt = np.nan
            ...
            except (ValueError, AttributeError, KeyError, TypeError, IndexError, RuntimeError) as e:
                skipped.append(
                    {
                        "year": year,
                        "event": ev_name,
    ...
    for year in range(start_year, end_year + 1):
        ...
        if only_specific and year in only_specific:
            df, skipped = build_profiles_for_season(
                year, circuit_metadata, only_specific=only_specific[year]
            )
        else:
...
def fit_track_clusters(
    ...
    # Aggregate per track
    track_features = df_profiles.groupby(group_cols)[feat_cols].mean().reset_index()
    X = track_features[feat_cols]

    # Build pipeline
    steps = [("imputer", SimpleImputer()), ("scaler", scaler or StandardScaler())]
    if do_pca:
...
def create_pca_for_n_clusters(
    circuits: pd.DataFrame, clusters: int, feat_cols: List[str]
    ...
    circuits["cluster"] = (circuits["event"].astype(str) + "_" + circuits["year"].astype(str)).map(
        key
    )
