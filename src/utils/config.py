...
class ProductionConfig:
    ...
    def get_qualifying_strategy(
        self,
        weekend_type: Literal['sprint', 'conventional']
    ) -> Dict:
        """
        ...
        """
        quali_config = self.config['qualifying_prediction']
    ...
    def get_expected_mae(
        ...
        if prediction_type == 'qualifying':
            if weekend_type:
                ...
                sprint = self.config['qualifying_prediction']['sprint_weekends']
                conv = self.config['qualifying_prediction']['conventional_weekends']
            ...
            return self.config['race_prediction']['default']['expected_mae']
        
        return 4.0  # Conservative fallback
    
    def should_use_method(self, method: str) -> bool:
        """Check if method should be tracked."""
        return method in self.config['performance_tracking']['methods_to_track']
    
    def get_performance_ranking(self) -> Dict:
        """Get performance ranking from comprehensive testing."""
        return self.config['notes']['performance_ranking'].copy()
    ...
    def __str__(self):
        ...
        for rank, info in self.config['notes']['performance_ranking'].items():
...
def load_production_config(config_path='production_config.json') -> ProductionConfig:
    """Load production config."""
...
if __name__ == '__main__':
    ...
    print(f"  Method: {sprint_strategy['method']}")
    print(f"  Session: {sprint_strategy['session']}")
    print(f"  Expected MAE: {sprint_strategy['expected_mae']:.2f}")
    
    # Conventional weekend
    conv_strategy = config.get_qualifying_strategy('conventional')
    print(f"\nConventional weekend strategy:")
    print(f"  Method: {conv_strategy['method']}")
    print(f"  Blend weight: {conv_strategy['blend_weight']}")
    print(f"  Expected MAE: {conv_strategy['expected_mae']:.2f}")
    
    # Expected MAEs
    print(f"\nExpected MAEs:")
    print(f"  Sprint quali: {config.get_expected_mae('qualifying', weekend_type='sprint'):.2f}")
    print(f"  Conv quali: {config.get_expected_mae('qualifying', weekend_type='conventional'):.2f}")
    print(f"  Overall quali: {config.get_expected_mae('qualifying'):.2f}")
    print(f"  Race: {config.get_expected_mae('race'):.2f}")
