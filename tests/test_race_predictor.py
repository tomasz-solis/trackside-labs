...
class TestRacePredictor:
    ...
    def test_initialization(self, mock_driver_chars, temp_data_dir):
        ...
        predictor = RacePredictor(year=2026, data_dir=temp_data_dir, driver_chars=mock_driver_chars)
    ...
    def test_predict_returns_valid_structure(
        self, mock_driver_chars, mock_qualifying_grid, temp_data_dir
    ):
    ...
    def test_predict_assigns_positions_correctly(
        self, mock_driver_chars, mock_qualifying_grid, temp_data_dir
    ):
    ...
    def test_lap_1_chaos_simulation(self, mock_driver_chars, temp_data_dir):
        ...
        for _ in range(30):
            pos = predictor._simulate_lap_1_chaos(start_pos=10, racecraft=0.8, consistency=0.8)
    ...
    def test_overtaking_difficulty_reduces_pace_gain(self, mock_driver_chars, temp_data_dir):
        ...
        gain_easy = predictor._calculate_effective_pace_gain(
            current_pos=10, pace_delta=pace_delta, difficulty=0.2, skill=skill
        )

        # Hard overtaking (Monaco)
        gain_hard = predictor._calculate_effective_pace_gain(
            current_pos=10, pace_delta=pace_delta, difficulty=0.9, skill=skill
        )
    ...
    def test_dnf_probability_increases_with_risk_factors(self, mock_driver_chars, temp_data_dir):
        ...
        dnf_safe = predictor._calculate_dnf_probability(
            team="Red Bull", consistency=0.95, weather="dry", track_info={"type": "permanent"}
        )

        # Risky scenario
        dnf_risky = predictor._calculate_dnf_probability(
            team="Cadillac", consistency=0.60, weather="rain", track_info={"type": "street"}
        )
...
class TestRacePredictorIntegration:
    ...
    def test_full_race_prediction_completes(
        self, mock_driver_chars, mock_qualifying_grid, temp_data_dir
    ):
